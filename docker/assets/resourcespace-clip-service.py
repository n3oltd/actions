...
EMBEDDING_DIM = 768
VECTOR_BLOB_BYTES = EMBEDDING_DIM * 4
...
db_host = os.getenv("CLIP_DB_HOST")
if not db_host:
    sys.exit("CLIP_DB_HOST must be set: the database is not local to this service.")
...
def load_vectors_for_db(db_name, force_reload=False):
    ...
    for ref, resource, blob in rows:
        ...
        if len(blob) != VECTOR_BLOB_BYTES:
            print(f"Skipping ref {ref} (resource {resource}): blob is {len(blob)} bytes, expected {VECTOR_BLOB_BYTES}")
            continue

        try:
            vector = np.frombuffer(blob, dtype=np.float32).copy()
            if vector.shape != (EMBEDDING_DIM,):
                print(f"Skipping ref {ref} (resource {resource}): vector shape {vector.shape}, expected ({EMBEDDING_DIM},)")
                continue
    ...
    if db_name in cached_vectors:
        ...
        vectors = np.stack(new_vectors) if new_vectors else np.empty((0, EMBEDDING_DIM), dtype=np.float32)
        ids = new_ids
...
async def search(
    ...
    try:
        ...
        if text:
            ...
            if not row or not row[0] or len(row[0]) != VECTOR_BLOB_BYTES:
...
async def tag_search(
    ...
    if url not in tag_vector_cache:
        try:
            ...
            if not vectors:
                raise ValueError(
                    f"No {EMBEDDING_DIM}-dimension tag vectors in {url}. A vocabulary "
                    f"built for a different model cannot be used; rebuild it with "
                    f"tagdb_build.php against {MODEL_NAME}."
                )
    ...
    if vector:
        try:
            ...
            if resource_vector.shape != (EMBEDDING_DIM,):
